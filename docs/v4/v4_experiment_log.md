...
| **V4 Trial 8 (当前最佳)** | **40.34** | **0.89** | **5.01** | **33.82** | α_mim=0.1软约束 + 2.0x div_loss + 28k后LR线性衰减 (帕累托全面突破) |
...
**执行结果**:
- *[执行状态]*: 训练与评测顺利完成 (2026-09-02)
- *[验证集 PPL]*: 38.21 (最佳验证集 Checkpoint `CASE_45999_38.2064`)
- *[测试集 PPL]*: **33.82** (全阶段历史最低 / 流畅度最优，超越原文 35.37 和 V3 的 34.54)
- *[测试集 Emo Acc]*: **40.34%** (超越原始 CASE 论文基线 40.20%)
- *[测试集 Dist-1]*: **0.89%** (超越原文 0.74% 与 V3 的 0.79%)
- *[测试集 Dist-2]*: **5.01%** (突破 5.0% 历史大关！超越原文 4.01% 整整 1.0 个百分点，超越 V3 的 4.06%)

**诊断分析与科学总结 (突破性成功)**:
1. **彻底打破帕累托前沿**：在之前的 Trial 1~7 中，多样性 (Dist-2) 与语言质量 (PPL / Emo Acc) 始终处于激烈互斥的跷跷板状态。Trial 8 成功达成了**四项核心指标的全面压制与历史最佳**：
   - 流畅度 (PPL: 33.82) 创全周期新低。
   - 生成多样性 (Dist-2: 5.01%) 首次突破 5.0% 门槛。
   - 情感分类 (Emo Acc: 40.34%) 稳步超越 CASE 原文。
2. **三元平衡机制的有效性验证**：
   - **`alpha_mim=0.1` 软约束**：成功避免了 Trial 7 中 `alpha_mim=0.5` 霸占解码器表征容量导致的词汇空间坍缩，同时维持了对解码器情感一致性的监督。
   - **`div_loss=2.0x` 适度推力**：相比 Trial 6 中 3.0x 的暴力拉扯与过拟合，2.0x 提供了恰到好处的多样性引导。
   - **28k 步后 LR 线性衰减**：彻底解决了后半程持续高学习率 (3.125e-4) 引发的泛化灾难与验证集漂移，验证集 PPL 从 Trial 7 的 39.89 平滑收敛至 38.21。
