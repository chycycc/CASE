# -*- coding: utf-8 -*-
"""
CASE-EPCL V8.2 B 全自动化实战执行流水线 (V8.2 B Automated Pipeline Runner)

功能涵盖:
1. 环境与磁盘安全审计 (RTX 3050 4GB 显存 / E 盘 > 10GB 空间审计)
2. 启动 V8.2 B 破局迭代实战训练:
   - [关键机理修复] 学习率余弦退火与冻结状态彻底解绑 (在 disable_freeze 下自 24k 步平滑衰减至 1e-5)
   - [早停机制升级] --patience 12 解除 5 次硬编码截断，确保走完 22,000 步完整余弦周期
...
def main():
    parser = argparse.ArgumentParser(description="CASE-EPCL V8.2 B 全自动化流水线")
    ...
    v8_2_b_dir = os.path.join(PROJECT_ROOT, "save", "epcl_v8_2_b")
    os.makedirs(v8_2_b_dir, exist_ok=True)
    train_log = os.path.join(v8_2_b_dir, "train.log")
    eval_log = os.path.join(v8_2_b_dir, "eval_pipeline.log")

    log_header("CASE-EPCL V8.2 B 破局迭代全自动执行流水线启动")
    check_disk_space(10.0)

    # 1. 训练命令配置 (解绑退火 + 动态早停耐心 12)
    cmd_train = [
        PYTHON_EXE, "main.py",
        "--dataset", "ED",
        "--woStrategy",
        ...
        "--save_path", "save/epcl_v8_2_b"
    ]

    # 执行训练
    if not args.only_eval:
        run_command_with_logging(cmd_train, train_log, desc="V8.2 B 破局实战训练 (解绑退火 + 动态早停)")

    # 磁盘维护与黄金权重查找
    best_ckpt = find_best_checkpoint(v8_2_b_dir)
    if best_ckpt:
        clean_redundant_checkpoints(v8_2_b_dir, best_ckpt)
        check_disk_space(10.0)
    ...
    if not args.skip_eval and best_ckpt:
        ...
        cmd_eval = [
            ...
            "--use_sparse_emo_bias",
            "--emo_vocab_topk_ratio", "0.15",
            "--plot_path", "docs/v8/images/v8_2_b_manifold.png"
        ]
        run_command_with_logging(cmd_eval, eval_log, desc="V8.2 B 自动化学术指标度量与流形评测")

    log_header("V8.2 B 全自动化流水线顺利执行完毕！")
